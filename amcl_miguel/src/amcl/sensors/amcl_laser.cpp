...
bool AMCLLaser::UpdateSensor(pf_t *pf, AMCLSensorData *data)
{
    ...
    else if(this->model_type == LASER_MODEL_LIKELIHOOD_FIELD)//#% tiene que ser mayor de 0.0 para actualizarse
...
double AMCLLaser::BeamModel(AMCLLaserData *data, pf_sample_set_t* set)
{
    ...
    //#% This one I have added to use laser_coeff;
...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)
{
    ...
    for (j = 0; j < set->sample_count; j++)
    {
        ...
        //#% comentado para que no me llene el terminal de los valores de los pesos
        //std::cout<<p<<endl;
        sample->weight *= p;
        total_weight += sample->weight;
    }
    ...
    self->pub_coeff_laser.publish(coeff_sensor_msg);
